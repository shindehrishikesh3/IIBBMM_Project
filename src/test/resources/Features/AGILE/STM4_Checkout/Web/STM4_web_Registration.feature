@Registration @Team4_Web @web
Feature: Web - Registration Page
	...
	Scenario Outline: New Privacy and Policy consent in Registration Page for AU/NZ
		...
		And user click on register link "Header.register_link"
  ...
  Scenario Outline: To verify updated Marketing Email Opt In Option on Registration Page for UK and DE
    And user click on register link "Header.register_link"
    And user wait for "3" seconds
    And user click on business account "Register.businessAccount_radio" only in "de" store/region
    And user wait for "3" seconds
    And user scroll till "Register.marketOptIn_text"
    And user wait for "2" seconds
    And user take screenshot of the page
    And user verify element "Register.marketOptIn_text" is present on page
    And user verify text marketing opt in "<value>" in page "Register.marketOptIn_text"
    And user take screenshot of the page
    ...
    @Sprint77 @STM4-2597 @CantAcceptKoreanCharacters @kr @live
  ...
  @sprint82 @STM4-2706 @live @ReduceZipCodeFieldLength
